...
public class SparkKafkaConsumer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length < 4) {
      System.err.println(
          "Usage: SparkKafkaWordCount <zkQuorum> <group> <topics> <numThreads>");
      System.exit(1);
    }
    ...
    JavaStreamingContext jssc =
        new JavaStreamingContext(sparkConf, new Duration(500));
        ...
        for (Tuple2<String, Double> tuple : tupleList) {
          if (tuple != null && tuple._1() != null && tuple._2() != null) {
            ...
            MongoDatabase database =
                mongoClient.getDatabase("airline").withCodecRegistry(
                    pojoCodecRegistry);

            MongoCollection<DelayModel> collection =
                database.getCollection("delay", DelayModel.class);
            List<String> arr = Arrays.asList(key.split(","));
            DelayModel delay = new DelayModel(arr.get(0), arr.get(2),
                                              arr.get(3), arr.get(4), value);
